...
with open(f"../Input_files/jh_cncb_daily_dates_{old_date}.pkl",'rb') as file: #Contains only country name for each date
    aggregated_mutations = pickle.load(file)

with open(f"../Input_files/jh_cncb_daily_dates_identifiers_{old_date}.pkl",'rb') as file:
    processed_identifiers = pickle.load(file)

# gff3 parsing

all_names = os.listdir(f'/gpfs/group/balch/data/gff3_cncb_{new_date}')
...
for identifier in tqdm(meta.index):
    ...
    #--------------------------
    with open(f'/gpfs/group/balch/data/gff3_cncb_{new_date}/{file_name}') as text_file:
    ...
    if counter<len(lines) and meta.loc[identifier,'Country'] in set(jh_data['Country_Region']): #country needs to be found in jh data for inf/fata rates
        ...
        gff = pd.read_csv(f'/gpfs/group/balch/data/gff3_cncb_{new_date}/{file_name}',sep='\t',skiprows=counter,usecols=[1,3,4,8],names=column_names)
...
with open(f"../Input_files/jh_cncb_daily_dates_{new_date}.pkl",'wb') as file:
    pickle.dump(aggregated_mutations,file,protocol=pickle.HIGHEST_PROTOCOL)
with open(f"../Input_files/jh_cncb_daily_dates_identifiers_{new_date}.pkl",'wb') as file:
    pickle.dump(processed_identifiers,file,protocol=pickle.HIGHEST_PROTOCOL)
with open(f"../Input_files/missing_{new_date}.pkl",'wb') as file:
