...
getMapSlice<-function(start,stop,prot,L,genomeLen,kriged_df,krige_df,genetab,AF_tab,dt,muts=F,minVar=F,IWM=F){
  ...
  if(IWM==T){
    minVar<-read.csv(paste0(prot,"_IWM_Z_3M_rgb_pymol_",dt,".csv"))
...
# get prediction value for each residue with inverse variance weighting

getIWM<-function(input,df_prot,pal){
  block<-df_prot[which(df_prot[,1]==input),]
  res<-block[1,]
  iwm<-weighted.mean(block$var1.pred,1/block$var1.var)
  ir_iwm<-block$IR[which.min(abs(block$var1.pred - iwm))]
  var_iwm<-block$var1.var[which.min(abs(block$var1.pred - iwm))]
  
  res$IR<-ir_iwm
  res$var1.pred<-iwm
  res$var1.var<-var_iwm
...
# given a protein, get a table of IWM - one per aa

getIWMZ<-function(start,stop,prot,L,genomeLen,kriged_df,dt,writeOut=T){
  ...
  out<-do.call("rbind",lapply(aa,function(x) getIWM(x,df_prot,pal)))
  
  out=cbind(1:nrow(out),out)
  colnames(out)[1]<-"residue"
  
  if(writeOut==T){
    write.csv(out,file=paste0(prot,"_IWM_Z_3M_rgb_pymol_",dt,".csv"),quote=F,row.names=F)
  }
  return(prot)
}
